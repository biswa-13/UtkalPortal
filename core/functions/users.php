...
mail($to, $subject, $body, 'From: Info - Utkal Placement <info@utkalplacement.org>');
}
...
function academic_data($academic_data) {
	array_walk($academic_data, 'array_sanitize');
	//$register_data['password'] = md5($register_data['password']);
	
	$fields = '`' .implode('`, `', array_keys($academic_data)) . '`';
	$data = '\'' . implode('\', \'', $academic_data) . '\'';
	
	mysql_query("INSERT INTO `academic` ($fields) VALUES ($data)");
}

function academic_data_update($user_id, $update_data) {
	$update = array();
	array_walk($update_data, 'array_sanitize');
	...
	mysql_query("UPDATE `academic` SET " . implode(', ', $update) . "WHERE `user_id` = $user_id" );
...
function edu_data($user_id){
	...
	if($func_num_args > 1) {
		...
		$fields = '`' . implode('`, `', $func_get_args) . '`';
		$data = mysql_fetch_assoc(mysql_query("SELECT $fields FROM `academic` WHERE `user_id`=$user_id"));
...
function edu_pg($user_id){
	$user_id = sanitize($user_id);
	return (mysql_result(mysql_query("SELECT COUNT(`pg_course`) FROM `academic` WHERE `user_id` = '$user_id'"), 0) == 1) ? true : false;
}

function user_edu($user_id){
	$user_id = sanitize($user_id);
	return (mysql_result(mysql_query("SELECT COUNT(`user_id`) FROM `academic` WHERE `user_id` = '$user_id'"), 0) == 1) ? true : false;
}

function user_cv($user_id){
	$user_id = sanitize($user_id);
	return (mysql_result(mysql_query("SELECT COUNT(`user_id`) FROM `user_cv` WHERE `user_id` = '$user_id'"), 0) == 1) ? true : false;
}
